...
void Show_map()
{
        int map[Y][X]={        			//建立原始地图,0 代表可移动区域，1 代表障碍☆☆☆☆☆☆
...
int WayFinding(int start_y,int start_x,int end_y,int end_x)
{
    ...
    // 判断是否停在过道上
    if(map[end_y][end_x] == 0)
    {
        cout << "您选择的停车位是过道，请重新选择！！！" <<endl;
        return 0;
    }
	...
	int goodnum = box_count;			// 保存个数，后面有用☆☆☆☆☆☆(回溯，计算所走的路径)
